 ...
 *
 * Usage (programmatic / CLI wrapper):
 *   import { buildGatewayApp } from './gateway/server';
 *   const { server, vault } = await buildGatewayApp();
 */
...
export async function buildGatewayApp(options: { port?: number; host?: string; healthCheckIntervalMs?: number } = {}): Promise<GatewayApp> {
    ...
    // Override port/host from options if provided
    ...
    return { server: app, vault, config };
}

// ─── Standalone Server Lifecycle ──────────────────────────────────────────────

async function shutdown(vault: MultiManager, server: FastifyInstance, signal: string) {
...
// ─── Allow standalone execution (e.g. via ts-node) ────────────────────────────
    ...
    (async () => {
        try {
            ...
            for (const p of providers) {
                ...
                const def = config.providers.find((x: any) => x.name === p);
            ...
            process.once('SIGTERM', () => shutdown(vault, server, 'SIGTERM'));
            process.once('SIGINT', () => shutdown(vault, server, 'SIGINT'));
        } catch (err) {
